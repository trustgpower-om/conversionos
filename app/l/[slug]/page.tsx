...
export default async function LandingPage({ params }: Props) {
  ...
  const { data } = await supabaseAdminClient
    .from('landing_pages')
    .select(
      'id, profile_id, slug, title, headline, subheadline, hero_image_url, cta_text, color_primary, custom_sections, form_fields',
    )
    .eq('slug', slug)
    .eq('is_active', true)
    .limit(1)
  ...
  const blocks = page.custom_sections ? parseCustomSections(page.custom_sections) : null
  const formFields = page.form_fields ? parseFormFields(page.form_fields) : null
      ...
      {hasBlocks ? (
        ...
        <div className="flex flex-1 flex-col items-center bg-zinc-50 dark:bg-black">
          ...
          >
            ...
            <LeadForm slug={page.slug} landingPageId={page.id} fields={formFields ?? undefined} />
          </section>
        </div>
      )}
    </>
